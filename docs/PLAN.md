...
- 챕터별 마지막에 **종합 퀴즈** (MVP 5문항, 이후 10~15문항으로 확장) — 심화 교육 해제 조건에 사용
...
| | `POST /edu/lessons/:id/quiz` | 레슨 퀴즈 채점 + 해설 + XP |
| | `GET·POST /edu/chapters/:id/final` | 종합퀴즈 조회·채점 + XP |
...
→ 현재 P1 전체 보상은 최대 380 XP로 Lv3에 도달한다. P2 시뮬레이션 보상을 더해
Lv5에 도달하도록 튜닝하고, 4개 챕터 종합퀴즈를 모두 통과하면 XP와 무관하게 심화를 해제한다.
...
**현재 상태 (2026-07-15):** P0와 P1의 API/UI/콘텐츠를 구현했다. P1은 4개
챕터·12개 레슨·56개 퀴즈 문항·32개 용어를 포함한다. P2는 DB 스키마와 화면
플레이스홀더만 있으며, P3는 성향 설문만 구현되어 Gemini 연동은 아직 없다.
...
- 금융 서비스 오인 방지 — 전 화면 교육 목적 고지를 유지하고 AI 가드레일은 P3에서 처음부터 포함
