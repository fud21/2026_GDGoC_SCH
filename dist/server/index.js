globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/flood-lab-B0eWv3xV.js","assets/layout-segment-context-D0Pj1Jfq.js"];
...
function getDraftSecret() {
	return "3cf4132d-d532-4086-8448-de4b445aadae";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "9da2bf26-ce87-4548-9a10-c2034a2f788f";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "9da2bf26-ce87-4548-9a10-c2034a2f788f") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "9da2bf26-ce87-4548-9a10-c2034a2f788f",
		rootBoundaryId
	});
}
/**
...
var FloodLab = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'FloodLab' is called on server");
}, "b8d6333b0bd5", "FloodLab");
//#endregion
//#region app/page.tsx
var page_exports = /* @__PURE__ */ __exportAll({ default: () => Home });
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(FloodLab, {});
}
...
async function generateMetadata() {
	...
	const description = "도시 공사와 하천 변경이 침수 위험에 미치는 영향을 공사 전에 시뮬레이션합니다.";
	return {
		title,
		description,
		openGraph: {
...
//#endregion
//#region app/api/simulate/route.ts
...
var safeNumber = (value, fallback, min, max) => {
	const parsed = typeof value === "number" ? value : Number(value);
	return Number.isFinite(parsed) ? clamp(parsed, min, max) : fallback;
};
var riverY = (x) => 11 + Math.sin(x * .31) * 1.7;
function normalizeInterventions(value) {
	if (!Array.isArray(value)) return [];
	return value.slice(0, 16).flatMap((item, index) => {
		...
		if (!candidate.type || ![
			...
			"culvert",
			"blockage"
		].includes(candidate.type)) return [];
		return [{
...
function calculate(input) {
	const forcing = clamp(input.rainfall / 140 * .46 + input.duration / 240 * .18 + input.discharge / 1500 * .28 + input.tide / 2.5 * .08, .08, 1.35);
	const cells = [];
	for (let y = 0; y < HEIGHT; y += 1) for (let x = 0; x < WIDTH; x += 1) {
		const distanceToRiver = Math.abs(y - riverY(x));
		const riverInfluence = Math.exp(-distanceToRiver / 4.7);
		const urbanPocket = Math.max(0, Math.sin(x * .55 + y * .38)) * Math.exp(-Math.abs(y - 6) / 7);
		const terrain = 5.4 + distanceToRiver * .28 + Math.sin(x * .22) * .38 + Math.cos(y * .51) * .18;
		let depth = forcing * (2.35 * riverInfluence + .62 * urbanPocket) - (.22 + distanceToRiver * .052);
		for (const intervention of input.interventions) {
			...
			if (intervention.type === "levee") {
				depth -= .76 * localWeight;
				if (x < intervention.x && distanceToRiver < 4.5) depth += .14 * Math.exp(-Math.abs(dx) / 7) * Math.exp(-Math.abs(dy) / 3);
			}
			if (intervention.type === "excavation") depth -= .42 * localWeight * Math.exp(-distanceToRiver / 5);
			if (intervention.type === "culvert") depth -= .58 * localWeight * (.65 + urbanPocket * .5);
			if (intervention.type === "blockage") {
				const upstream = x <= intervention.x ? 1 : -.25;
				depth += .72 * upstream * Math.exp(-Math.abs(dx) / 7.5) * Math.exp(-Math.abs(dy) / 3.4);
			}
		}
		depth = clamp(depth, 0, 3.8);
		const arrival = depth < .08 ? 99 : clamp(Math.round(2 + distanceToRiver * .72 + (1.15 - forcing) * 4), 1, 11);
		cells.push({
			x,
			y,
	...
	const getDepth = (x, y) => cells[y * WIDTH + x]?.depth ?? 0;
	const maxDepth = Math.max(...cells.map((cell) => cell.depth));
	const floodedCells = cells.filter((cell) => cell.depth >= .15);
	const affectedBuildings = BUILDINGS.filter(([x, y]) => getDepth(x, y) >= .2).length;
	const exposedRoads = cells.filter((cell) => (cell.x === 8 || cell.x === 20 || cell.y === 5 || cell.y === 17) && cell.depth >= .18).length;
	const earliestArrivalStep = Math.min(...cells.filter((cell) => cell.depth >= .3).map((cell) => cell.arrival), 12);
	const riskScore = clamp(Math.round(floodedCells.length / cells.length * 58 + maxDepth / 3.8 * 42), 0, 100);
	return {
		cells,
		metrics: {
			maxDepth: Number(maxDepth.toFixed(2)),
			floodedArea: Number((floodedCells.length * .0025).toFixed(2)),
			affectedBuildings,
			exposedRoads: Number((exposedRoads * .05).toFixed(1)),
			firstArrivalMinutes: earliestArrivalStep * 15,
			riskScore,
			estimatedDamage: Math.round(affectedBuildings * 2.8 + floodedCells.length * .018)
		}
	};
}
async function POST(request) {
	...
	try {
		...
		return Response.json({ message: "시뮬레이션 입력 형식이 올바르지 않습니다." }, { status: 400 });
	}
	const input = {
		rainfall: safeNumber(body.rainfall, 96, 10, 180),
		duration: safeNumber(body.duration, 180, 30, 360),
		discharge: safeNumber(body.discharge, 980, 100, 2e3),
		tide: safeNumber(body.tide, .8, 0, 3),
	...
	return Response.json({
		...
		comparison: {
			...
			deltaDamage: scenario.metrics.estimatedDamage - baseline.metrics.estimatedDamage
		},
...
var routes = [{
	...
	ids: {
		"route": "route:/api/simulate",
		"page": null,
		"routeHandler": "route-handler:/api/simulate",
		"rootBoundary": "root-boundary:/",
		"layouts": ["layout:/"],
		"templates": [],
		"slots": {}
