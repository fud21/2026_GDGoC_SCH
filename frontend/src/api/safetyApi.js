...
// 백엔드 dataType(cctv/보안등/파출소) -> mockSafety.TYPE_META 키
const DATA_TYPE_TO_TYPE = { cctv: 'cctv', 보안등: 'light', 파출소: 'police' };
...
//
// 주의: 백엔드는 환경조명(밝기)·야간 통행량은 내려주지 않는다.
// 없는 값은 지어내지 않고 traffic만 임시 고정값("보통")을 쓴다.
export function toAnalysisResult(apiResponse) {
  const { address, lat, lng, grade, score, details, crime, nearbyFacilities } = apiResponse;
  ...
  return {
    ...
    facilities: (nearbyFacilities || []).map((f) => ({
      type: DATA_TYPE_TO_TYPE[f.dataType] || f.dataType,
      name: f.label,
